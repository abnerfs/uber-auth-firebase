...
const link = process.env.BASEURL || `http://localhost:${PORT}/`;
const redirect_uri = link + 'callback';
const scope = "delivery history history_lite places profile";

/* Firebase Admin Stuff 
    Admin credentials: https://firebase.google.com/docs/admin/setup?hl=pt-br#add_firebase_to_your_app
*/
    ...
    */
    ...
    fetch('https://login.uber.com/oauth/v2/token', {
        ...
        .then(response => {
            ...
            .then(responseMe => {
                if(responseMe.code === 'unauthorized')
                {
                    res.send(responseMe);
                    return;
                }
                ...
                createAccount(uuid, `${first_name} ${last_name}`, picture, access_token)
                    .then(firebaseToken =>{
...
const createAccount = (userID, displayName, photoUrl, accessToken) => {

    const uid = `uber:${userID}`;
    const refNode = `uberAccessToken/${uid}`;
    console.log(refNode);

    /* Save token */
    const saveToken = admin.database().ref(refNode)
        .set(accessToken);
    

    /* Create or update user */
    const userCredentials = admin.auth().updateUser(uid, {
        displayName,
        photoUrl
    }).catch(error => {
        ...
        /* throw if another error */
        throw error;
    });
    
    return Promise.all([saveToken, userCredentials]).then(res => {
        return admin.auth().createCustomToken(uid)
                .then(token => {
                    console.log(`Created Token ${token} for user ${uid}`);
                    return token;
                })
    });
}